    ...
    try {
      ...
      if (data.length > 0) {
        ...
        const fixedData = data.map((item, index) => ({
          ...item,
          amount: item.amount ?? (index === installmentCount - 1 ? installmentAmount + remaining : installmentAmount),
          dueDate:
            item.dueDate ??
  ...
  const generateSchedule = async () => {
    ...
    for (let i = 0; i < installmentCount; i++) {
      ...
      schedule.push({
        ...
        amount: i === installmentCount - 1 ? installmentAmount + remaining : installmentAmount,
